...
| File | What it is |
|---|---|
| `verified-loans.csv` | **the verified loan dataset** — all 442 sealed loans across the 26 canonical fields, each with its `version` and `recordHash` |
...
`audit-trail.sample.jsonl` holds 120 of 1,108 events and `verified-records.sample.json`
holds 25 of 442 — the full versions are around 1MB each and add nothing a reader cannot see in
the first few. `attestation.json` keeps its real root and signer with the leaf list
trimmed to 20 of 442, noted inline.
